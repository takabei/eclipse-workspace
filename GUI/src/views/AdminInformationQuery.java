...
//              //int col=table.getSelectedColumn();//选中列
				if(row != -1) {
					...
					saddress.setText(jTable1.getValueAt(row, 4).toString());					
				}
			}		
		});
...
//              //int col=table.getSelectedColumn();//选中列
		...
		sconfirm.addActionListener(new ActionListener() {			
			...
			public void actionPerformed(ActionEvent e) {
				...
				test.setTel(sstel);
				Class clazz = classService.findClass(sclazz);
				test.setClazz(clazz);
				int num = 0;
		...
		sdelete.addActionListener(new ActionListener() {			
			...
			public void actionPerformed(ActionEvent e) {
				...
				int num = 0;
				if(sstuId.equals("") ) {
					JOptionPane.showMessageDialog(AdminInformationQuery.this,"请填写学生Id");
				} else if((num = userService.findStudent(sstuId)) == 0) {
					JOptionPane.showMessageDialog(AdminInformationQuery.this,"不存在这个学生");
				} else if((num = userService.deleteStudent(sstuId)) == 0) {
					JOptionPane.showMessageDialog(AdminInformationQuery.this,"操作失败！请稍候重试");
				} else {
					JOptionPane.showMessageDialog(AdminInformationQuery.this,"操作成功！");
					InitTable1();
		...
		sselect.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				String sstuId = stuId.getText().trim();
				Student stu = null;
				...
				if(sstuId.equals("") ) {
					...
					JOptionPane.showMessageDialog(AdminInformationQuery.this,"不存在这个学生");
				} else if((stu = userService.getStudent(sstuId)) != null){
					stuId.setText(stu.getStuId());
					sname.setText(stu.getName());
					ssex.setText(stu.getSex());
					stel.setText(stu.getTel());
					saddress.setText(stu.getAddress());	
				}
			}
		});
